...
class DetectLandingPad:
    ...
    def run(
        ...
        predictions = self.__model.predict(image, conf=0.7, device="cpu",boxes = True)
        ...
        image_annotated = predictions[].plot(conf=True)


        # Detach the xyxy boxes to make a copy,
        # move the copy into CPU space,
        # and convert to a numpy array
        boxes_cpu = predictions.boxes.cpu().numpy()
        
        # Get the xyxy boxes list from the Boxes object in the Result object
        boxes_xyxys = boxes_cpu.xyxy
        ...
        for i in range(boxes_cpu.shape[0]):
            ...
            print("suspects ERROR before creating the bounding boxes")
            print(x1,y1, x2,y2)
