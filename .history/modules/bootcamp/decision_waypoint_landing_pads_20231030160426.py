...
class DecisionWaypointLandingPads(base_decision.BaseDecision):
    ...
    def run(
        ...
        def get_set_relative_distance_command(
            ...
            #I put the minus operator at the place ^ for a previous try, it doesn't work since subtracting boolean values is not accepted behavior in p
            try:
        ...
        if self.arrived_at_waypoint == False:
            ...
            nearest_landing_pad_result = calculate_nearest_landing_pad(self.waypoint,landing_pad_locations)
            if validate_arrival_at_destination(report.position, nearest_landing_pad_result):
                print("after validation true")
                return land_command
            
            else:
                print("after validation false")
                set_relative_destination_command_result = (get_set_relative_distance_command(report.position,nearest_landing_pad_result,60, 60))
                return set_relative_destination_command_result
