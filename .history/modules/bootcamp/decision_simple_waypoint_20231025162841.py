...
class DecisionSimpleWaypoint(base_decision.BaseDecision):
    ...
    def run(
        ...
        def get_set_relative_distance_command(relative_flight_boundary_x_abs: float, relative_flight_boundary_y_abs: float)-> commands.Command:
            ...
            x_distance = x_distance if abs(x_distance) < abs(relative_flight_boundary_x_abs) else (relative_flight_boundary_x_abs*sign_x_distance)
            y_distance = y_distance if abs(y_distance) < abs(relative_flight_boundary_y_abs) else (relative_flight_boundary_y_abs*sign_y_distance)
            
            set_relative_destination_command = commands.Command.create_set_relative_destination_command(
                        x_distance,
                        y_distance,
                    )
        ...
        if report.status == drone_status.DroneStatus.HALTED:
            if report.position == report.destination:
                ...
                set_relative_destination_command = get_set_relative_distance_command(60,60)
                return set_relative_destination_command
