...
class DetectLandingPad:
    ...
    def run(self, image: np.ndarray) -> "tuple[list[bounding_box.BoundingBox], np.ndarray]":
        ...
        # * device
        # * verbose
        self.__model.predict(image, conf=0.7, device=self.__DEVICE, verbose=False)

        # Get the Result object
        prediction = self.__model(image)

        # Plot the annotated image from the Result object
        # Include the confidence value
        image_annotated = prediction[0].plot()

        # Get the xyxy boxes list from the Boxes object in the Result object
        boxes_xyxy = prediction[0].boxes.xyxy
        ...
        bounding_boxes = []
        for result in enumerate(boxes_cpu):
            bounding_boxes.append(bounding_box.BoundingBox.create(result[1])[1])
