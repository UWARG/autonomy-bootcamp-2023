...
class DetectLandingPad:
    ...
    def run(self, image: np.ndarray) -> "tuple[list[bounding_box.BoundingBox], np.ndarray]":
        ...
        # * source : Specifies the data source for inference.
        # Can be an image path, video file, directory, URL, or device ID for live feeds.
        # Supports a wide range of formats and sources, enabling flexible application across
        ...
        # Allows users to select between CPU, a specific GPU, or other compute devices for
        # model execution.

        # * verbose : Controls whether to display detailed inference logs in the terminal,
        # providing real-time feedback on the prediction process.

        predictions = self.__model.predict(
            source=image, conf=0.70, device=self.__DEVICE, verbose=False
        )
        ...
        for i in range(boxes_cpu.shape[0]):
            result, box = bounding_box.BoundingBox.create(bounds=boxes_cpu[i][:4])
            if result:
