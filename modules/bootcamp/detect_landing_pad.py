...
class DetectLandingPad:
    ...
    def run(self, image: np.ndarray) -> "tuple[list[bounding_box.BoundingBox], np.ndarray]":
        ...
        # * device
        # * verbose
        predictions = self.__model.predict(image, conf = 0.5, device = self.__DEVICE, verbose = False)
        ...
        for i in range(0, len(boxes_cpu)):
            box_info = boxes_cpu[i]
            result, box = bounding_box.BoundingBox.create(box_info)
