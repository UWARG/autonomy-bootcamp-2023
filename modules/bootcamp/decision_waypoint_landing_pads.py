...
class DecisionWaypointLandingPads(base_decision.BaseDecision):
    ...
    def run(
        ...
        if self.waypoint_status:  # if at waypoint
            ...
            if report.status == drone_status.DroneStatus.HALTED:
                self.nearest_landing_pad = self.find_nearest_landing_pad(
                    position, landing_pad_locations
                )
                if (
                    self.get_relative_distance(position, self.nearest_landing_pad)
                    > self.acceptance_radius_sq
                ):
            ...
            if report.status == drone_status.DroneStatus.HALTED:
                if (
                    self.get_relative_distance(position, self.waypoint)
                    > self.acceptance_radius_sq
                ):
                    command = commands.Command.create_set_relative_destination_command(
    ...
    def get_relative_distance(
        self, position: location.Location, destination: location.Location
    ) -> float:
        return (position.location_x - destination.location_x) ** 2 + (
            position.location_y - destination.location_y
        ) ** 2
