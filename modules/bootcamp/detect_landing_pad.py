...
class DetectLandingPad:
    ...
    def run(self, image: np.ndarray) -> "tuple[list[bounding_box.BoundingBox], np.ndarray]":
        ...
        # * device
        # * verbose
        predictions = self.__model.predict(source=image, conf=0.5, device=self.__DEVICE, verbose=True)
        ...
        for i in range (boxes_cpu.shape[0]):
            result, bounding_Box_obj = bounding_box.BoundingBox.create(boxes_cpu[i])
            if (result):
