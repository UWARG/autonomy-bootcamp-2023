...
class DecisionWaypointLandingPads(base_decision.BaseDecision):
    ...
    def run(
        ...
        def closest_landing_pad(report: drone_report.DroneReport, landing_pad_locations: "list[location.Location]") -> location.Location:
            nearest_landing_pad = None
            nearest_distance_squared = float("inf")
            for landing_pad in landing_pad_locations:
                distance_squared = (report.position.location_x - landing_pad.location_x) ** 2 + (report.position.location_y - landing_pad.location_y) ** 2
        ...
        if report.status == drone_status.DroneStatus.HALTED and self.waypoint_reached is False:
            radius_away_squared = (report.position.location_x - self.waypoint.location_x) ** 2 + (report.position.location_y - self.waypoint.location_y) ** 2
            ...
            command = commands.Command.create_set_relative_destination_command(nearest_landing_pad.location_x - report.position.location_x, nearest_landing_pad.location_y - report.position.location_y)
            if (report.position.location_x - nearest_landing_pad.location_x) ** 2 + (report.position.location_y - nearest_landing_pad.location_y) ** 2 <= self.acceptance_radius ** 2:
