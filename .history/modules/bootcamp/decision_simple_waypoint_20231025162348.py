...
class DecisionSimpleWaypoint(base_decision.BaseDecision):
    ...
    def run(
        ...
        def inner_function(relative_flight_boundary_x_abs: float, relative_flight_boundary_y_abs: float)-> "tuple[float, float]":
                ...
                sign_y_distance = (x_distance > 0) - (x_distance < 0)
            
                x_distance = x_distance if abs(x_distance) < abs(relative_flight_boundary_x_abs) else relative_flight_boundary_x_abs*sign of x_distance
